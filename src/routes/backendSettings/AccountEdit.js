...
import {ResetBackendPasswordModal} from './components';
...
class AccountEdit extends React.Component {
	...
	componentDidMount () {
		...
		if (!id) {
			...
			this.props.dispatch({
				type: 'backend/queryBackendAccountInfo',
				payload: {
					id: id,
					cb: this.queryCb
				}
	...
	onSubmit () {
		...
		validateFields((errors) => {
			...
			this.props.dispatch({
				type: 'backend/editBackendAccount',
				payload: {
					id: editRecord.id,
					params: {
						...getFieldsValue()
	...
	submitCb (response) {
		if (response.code != 'SUCCESS') {
			...
			this.context.router.push('/backend/account/list');
		}
	}

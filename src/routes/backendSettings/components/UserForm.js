...
class RoleForm extends React.Component {
	...
	componentDidMount () {
		this.props.dispatch({
			type: 'administrator/queryDepartments'
		});
		this.props.dispatch({
	...
	componentWillReceiveProps (newProps, preProps) {
		if (newProps.setting.roles && !this.props.setting.roles) {
			this.setState({
				checkedPermissions: getPermissionsByRole(newProps.setting.roles, this.state.checkedRoles)
			});
		}
	}
			...
			<div>
				...
				<Row className="formSection">
					<Col className={styles.roles}>
						...
						{ (!this.props.roleLoading && this.props.setting.roles) &&
							<Checkbox.Group options={this.props.setting.roles} defaultValue={checkedRoles} onChange={this.onSelectRoleChange} />
						}
					</Col>
				</Row>
				<h3 className="formSectionTitle">
				...
				<PermissionList checkedPermissions={this.state.checkedPermissions} showAll />
...
function mapStateToProps({ administrator, setting, loading }) {
	return { administrator, setting, loading: loading.models.administrator, roleLoading: loading.models.setting };
}

...
class AccountList extends React.Component {
	...
	render() {
		...
		const columns = [
			{
				...
				render: (text, record) => {
					return (
						<div>
							<Tooltip title="修改">
								<Link to={`/backend/account/${record.id}`}><Icon type="edit" /></Link>
							</Tooltip>
		...
		const paginationConf = this.props.backendAccount.accounts_pagination;
		let pagination;
		if (paginationConf) {
			pagination = {
				...paginationConf
		...
		return (
			<Card bodyStyle={{ padding: '24px 24px 32px 24px' }}>
				...
				<Table columns={columns} dataSource={ this.props.backendAccount.accounts } bordered loading={this.props.loading} pagination={pagination} />
			</Card>
		);
	};
}
